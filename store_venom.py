...
def store_weapon_data(item_id, weapon_name, weapon_data):
    if item_id:
        ...
        if 'removed' in weapon_data:
            ...
            removed.append(weapon_data['ankama_id'])
            print 'Setting as REMOVED: %s' % old_item.name
        elif weapon_data['w_type'] == 'Ceremonial item':
            fake_delete_item(item_id, True)
            print 'REMOVED for being Ceremonial: %s' % old_item.name
        ...
        if weapon_data.get('w_type') == 'Ceremonial item':
            print 'Did not insert ceremonial item: %s' % weapon_data['name']
            return
        if 'removed' in weapon_data:
            print 'Did not insert removed item: %s' % weapon_data['ankama_id']
            removed.append(weapon_data['ankama_id'])
            return
...
def get_item_id_and_name_for_ankama_profile(c, entities_table, ankama_profile, touch=False):
    if ankama_profile[1] == 'set':
        ...
        if touch:
            ...
            if query_result is None:
                c.execute('SELECT id, name FROM %s WHERE ankama_id = ? AND ankama_type = ? and dofustouch = ?' % entities_table,
                          (ankama_profile[0], ankama_profile[1], 0))
                query_result = c.fetchone()
...
def _convert_json_item_to_item(json_item):
    ...
    for stat in json_item['stats']:
        if stat[2].startswith('(') and stat[2] != '(':
            ...
            if stat[2] == 'HP':
                stat[2] = 'Initiative'
            elif stat[2] == 'MP Parry':
                stat[2] = 'MP Loss Resist'
            elif stat[2] == 'AP Parry':
                stat[2] = 'AP Loss Resist'
